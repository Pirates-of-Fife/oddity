...
func _ready() -> void:
	var error_enter : Error = self.connect("_on_body_entered", body_entered)
	var error_exit : Error = self.connect("_on_body_exited", body_exited)
...
func _process(delta: float) -> void:
	...
	if cargo_in_area.is_being_held:
		$MeshInstance3D.show()
	else:
		$MeshInstance3D.hide()
