...
func spawn_gas_giant_sectors() -> void:
	...
	var sectors : Array = object_scatter.generate_gas_giant_scatter_sectors(splits, count_per_multi_mesh, planet_radius, scale_multiplier, invert_scale, scale_variance, density)
	var mesh : ArrayMesh = mineable_resource.low_detail_mesh
