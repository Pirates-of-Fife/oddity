...
func regen_mesh(planet_data : PlanetData) ->void:
	var arrs :Array= []
	arrs.resize(Mesh.ARRAY_MAX)
	
	var vertexArr :PackedVector3Array= PackedVector3Array()
	var uvArr :PackedVector2Array= PackedVector2Array()
	var normalArr :PackedVector3Array= PackedVector3Array()
	var indexArr :PackedInt32Array= PackedInt32Array()
	
	var resolution :int= planet_data.res
	...
	for y:int in range (resolution):
		for x:int in range (resolution):
			...
			var pointOnUnitSphere :Vector3 = pointOnUnitCube.normalized() * float(planet_data.radius)
	...
	for a:int in range(0, indexArr.size(), 3):
		var b : int = a + 1
		var c : int = a + 2
	...
	for i:int in range(normalArr.size()):
...
func _update_mesh(arrs : Array, planet_data : PlanetData) ->void:
